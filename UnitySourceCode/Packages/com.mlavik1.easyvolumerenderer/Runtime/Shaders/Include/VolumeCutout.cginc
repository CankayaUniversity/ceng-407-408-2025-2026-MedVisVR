...
// Quest 3 optimizasyonu: cross-section limiti 8 -> 2 (mobil GPU icin matris islem yukunu azaltir)
#define MAX_CROSS_SECTIONS 2
float4x4 _CrossSectionMatrices[MAX_CROSS_SECTIONS];
float _CrossSectionTypes[MAX_CROSS_SECTIONS];
...
bool IsCutout(float3 currPos)
{
    ...
    bool clipped = false;
    int sectionCount = min(_NumCrossSections, MAX_CROSS_SECTIONS);
    for (int i = 0; i < sectionCount && !clipped; ++i)
    {
