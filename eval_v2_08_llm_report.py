...
benchmarks = [
    ...
    ("ARC-Challenge",       65.0, 59.5, 60.0, 75.7),
    ("TruthfulQA",          54.2, 44.0, 42.3, 65.0),
]
bench_names  = [b[0] for b in benchmarks]
q_vals       = [b[1] for b in benchmarks]
llama_vals   = [b[2] for b in benchmarks]
mistral_vals = [b[3] for b in benchmarks]
gpt_vals     = [b[4] for b in benchmarks]

x = np.arange(len(bench_names)); w = 0.20
fig, ax = plt.subplots(figsize=(14, 5.5))
ax.bar(x-1.5*w, q_vals,       w, label="Qwen2.5-7B-Instruct (CARVIS)", color="#2563eb", edgecolor="white")
...
ax.set_title(
    ...
    "Source: Qwen2.5 Technical Report (arXiv:2412.15115, 2024).  "
    "CARVIS uses Q4_K_M quantised local inference via llama.cpp (offline, no API).",
    fontsize=10.5, fontweight="bold", pad=14)
ax.legend(fontsize=9, framealpha=0.85, ncol=4, loc="upper left")
fig.tight_layout()
p = OUT / "llm_benchmark_v2.png"
fig.savefig(p, dpi=180, bbox_inches="tight"); plt.close(fig)
print(f"Saved: {p}")

REPORT_SECTIONS = [
    "Clinical Summary", "Imaging Findings", "Tumour Characteristics",
    "Anatomical Localisation",
    "Differential Diagnosis", "Recommendations", "Clinical Note",
]

CLINICAL_TERMS = [
    "tumour","tumor","lesion","neoplasm","glioma","glioblastoma",
    "metastasis","enhancement","oedema","edema","necrosis","mass",
    "infiltration","resection","biopsy","MRI","CT","hemisphere","lobe",
    "parenchyma","ventricle","midline","prognosis","treatment",
    "radiotherapy","chemotherapy","liver","hepatic","cirrhosis",
    "hepatocellular","lung","pulmonary","tumour","nodule","consolidation",
...
SECTION_ALIASES = {
    ...
    "Anatomical Localisation": ["anatomical localisation","anatomical localization","atlas localisation","localisation","anatomy","lobe","hemisphere"],
...
def section_score(text, sec):
    return 1 if any(kw in text.lower() for kw in SECTION_ALIASES[sec]) else 0

def clinical_density(text):
    words = text.lower().split()
    if not words: return 0.0
    return sum(1 for w in words if any(t in w for t in CLINICAL_TERMS)) / len(words) * 100
...
if report_rows:
    ...
    for i in range(len(report_rows)):
        for j in range(len(REPORT_SECTIONS)):
            ...
            ax.text(j, i, "yes" if v else "—", ha="center", va="center",
                    fontsize=8.5, fontweight="bold", color="white" if v else "#9ca3af")
    prev = None
    for i, r in enumerate(report_rows):
        if r["organ"] != prev and prev is not None:
            ax.axhline(i-0.5, color="white", linewidth=2)
        prev = r["organ"]
    fig.colorbar(im, ax=ax, shrink=0.6, label="Section present (1=yes)")
    patches = [
        mpatches.Patch(color="#3b82f6", label="Brain MRI"),
        mpatches.Patch(color="#16a34a", label="Liver CT (n=1 GT †)"),
        mpatches.Patch(color="#ca8a04", label="Lung CT (inference only, no GT metrics)"),
    ]
    ax.legend(handles=patches, loc="lower right", fontsize=8,
              bbox_to_anchor=(1.14, -0.02), framealpha=0.85)
    ax.set_title("Report Structural Completeness — Section Coverage per Case (v2)\n"
                 "Note: 'Anatomical Localisation' replaces previous 'Atlas/Couinaud' label",
                 fontsize=11, fontweight="bold", pad=14)
    fig.tight_layout()
    p = OUT / "report_quality_heatmap_v2.png"
    fig.savefig(p, dpi=180, bbox_inches="tight"); plt.close(fig)
    print(f"Saved: {p}")

    short_ids = [r["id"].replace("BraTS20_Validation_","BV_") for r in report_rows]
    cols = [("#3b82f6" if r["organ"]=="brain"
              else "#16a34a" if r["organ"]=="liver" else "#ca8a04") for r in report_rows]
    wc   = [r["word_count"]    for r in report_rows]
    comp = [r["completeness"]  for r in report_rows]
    dens = [r["density"]       for r in report_rows]

    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    for ax_, vals, xlabel, title, mean_lbl in [
        (axes[0], wc,   "Word count",        "Report Word Count",          f"Mean: {np.mean(wc):.0f}"),
        (axes[1], comp, "Completeness (%)",   "Section Completeness (%)",   f"Mean: {np.mean(comp):.0f}%"),
        (axes[2], dens, "Clinical terms (%)", "Clinical Term Density (%)",  f"Mean: {np.mean(dens):.1f}%"),
    ]:
        ax_.barh(short_ids, vals, color=cols, edgecolor="white")
        ax_.set_xlabel(xlabel, fontsize=9)
        ax_.set_title(title, fontsize=10, fontweight="bold")
        ax_.axvline(np.mean(vals), color="#dc2626", linestyle="--", linewidth=1, label=mean_lbl)
    ...
    patches = [
        mpatches.Patch(color="#3b82f6", label="Brain MRI"),
        mpatches.Patch(color="#16a34a", label="Liver CT (n=1 GT)"),
        mpatches.Patch(color="#ca8a04", label="Lung CT (inference only)"),
    ]
    fig.legend(handles=patches, loc="lower center", ncol=3, fontsize=9,
               framealpha=0.85, bbox_to_anchor=(0.5,-0.04))
    fig.suptitle("CARVIS — Report Quality Metrics (v2)", fontsize=13, fontweight="bold")
    fig.tight_layout(); fig.subplots_adjust(bottom=0.15)
    p = OUT / "report_quality_bars_v2.png"
    fig.savefig(p, dpi=180, bbox_inches="tight"); plt.close(fig)
    print(f"Saved: {p}")
...
COMPONENTS = [
    ...
    ("Anatomical Labelling", None),
    ("RAG Evidence",       None),
]
comp_labels = [c[0] for c in COMPONENTS]
comp_matrix = []
case_ids_sys, case_organs = [], []
...
for case_dir in sorted(CASES.iterdir()):
    ...
    for comp_name, fname in COMPONENTS:
        ...
        elif comp_name == "Anatomical Labelling":
            tm = case_dir/"tumor_metrics.json"
            has = False
            if tm.exists():
                try:
                    d = json.loads(tm.read_text(encoding="utf-8-sig"))
                    has = any("anatomy" in l for l in d.get("lesion_list",[]))
                except: pass
            row.append(1.0 if has else 0.5)
        elif comp_name == "RAG Evidence":
            qa = case_dir/"qa_results.jsonl"
            has_rag = False
            if qa.exists():
                try:
                    rows_qa = [json.loads(l) for l in qa.read_text(encoding="utf-8-sig").splitlines() if l.strip()]
                    has_rag = any(r.get("evidence_ids") or r.get("evidence") for r in rows_qa)
                except: pass
            row.append(1.0 if has_rag else (0.5 if qa.exists() else 0.0))

    comp_matrix.append(row)
    case_ids_sys.append(case_dir.name.replace("BraTS20_Validation_","BV_"))
    case_organs.append(organ)

mat2 = np.array(comp_matrix)
import matplotlib.colors as mcols
cmap = mcols.LinearSegmentedColormap.from_list("traffic",["#fee2e2","#fef9c3","#dcfce7"])
...
labels_v = {1.0:"Active", 0.5:"Partial", 0.0:"—"}
for i in range(len(case_ids_sys)):
    for j in range(len(comp_labels)):
        v = mat2[i,j]
        ax.text(j, i, labels_v.get(v, f"{v:.0f}"), ha="center", va="center",
                fontsize=8, fontweight="bold", color="#1a1a2e" if v<0.8 else "#166534")
prev = None
...
for i, org in enumerate(case_organs): organ_groups[org].append(i)
for org, idxs in organ_groups.items():
    suffix = "" if org != "lung" else " (inf.)"
    ax.text(1.01, np.mean(idxs), org.title()+suffix, transform=trans,
            va="center", ha="left", fontsize=8.5, fontweight="bold",
            color=org_color_map.get(org,"#374151"))

fig.colorbar(im, ax=ax, shrink=0.5, ticks=[0,0.5,1],
             label="0=Inactive  0.5=Partial  1=Active")
ax.set_title("CARVIS — AI System Component Usage per Case (v2)\n"
             "Lung CT: inference only — no ground truth, no segmentation metrics",
             fontsize=12, fontweight="bold", pad=14)
fig.tight_layout()
p = OUT / "system_components_heatmap_v2.png"
fig.savefig(p, dpi=180, bbox_inches="tight"); plt.close(fig)
print(f"Saved: {p}")

print("\nDone. eval_v2_08_llm_report.py complete.")
